...
public class cartItemIO {
    ...
    public static List<?> selectItemsforsellerSearch (long Id,String ordername)
    {
        EntityManager em = emf.createEntityManager();
        EntityTransaction transaction = em.getTransaction();
        try {
            ...
            return em.createQuery("SELECT c.productByProductId.id,c.productByProductId.title, c.cartByCartId.accountByAccountId.phone" +
                    ",c.productByProductId.updatedAt,c.price, c.productByProductId.status,c.amount FROM" +
                    " CartItem c WHERE c.productByProductId.shopByShopId.accountByAccountId.id =?1 and c.productByProductId.title like  ?2").setParameter(1,Id).setParameter(2,"%"+ordername+"%").getResultList();
        } catch (Exception e)
        {
            System.out.println(e.getMessage());
            return null;
